...
import { coreContract, dssContract, dssContractAddress,dssVerifyContract,dssVerifyContractAddress } from "@/utils/contract/contract";
...
export async function registerOperator(aggregatorURL: string, operatorPubkey: string, operatorUrl: string) {
	...
	const registeredInVerifyDSS = await isRegisteredInVerifyDSS(operatorPubkey);
	if (!registeredInDSS) await registerInDSS();
	if(!registeredInVerifyDSS) await registerInVerifyDSS();
...
async function isRegisteredInVerifyDSS(operatorAddress: string): Promise<boolean> {
	const isRegistered = (await dssVerifyContract.read.isOperatorRegistered([operatorAddress])) as boolean;
...
async function registerInVerifyDSS() {
	await coreContract.write.registerOperatorToDSS([dssVerifyContractAddress, "0x"]);
